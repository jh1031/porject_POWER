...
const RecommendDirector = ({director}) => {
  ...
  const fahctData = async () => {
    try {
      const respones = await baseApi.get(`/person/${director[0].id}/movie_credits`);
    ...
    return (
        <div id="RecommendDirector">
            <ul>
                {crewMovies.map(item=><MovieItem key={item.id} movie={item}/>)}
            </ul>
            
            <div className="recommend-more">
                <button onClick={handleClickDirector}><img src="/icon/plus_128px.png" alt='해당 감독 더보기'/>
